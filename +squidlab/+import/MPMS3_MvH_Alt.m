...
classdef MPMS3_MvH_Alt < squidlab.import.LineReadingImportPipeline
       
    properties
        HeaderLines = 30; % We want to ignore this many lines of the file as header.
    end
    ...
    methods(Access = protected) 
        ...
        function [meta] = GetMetaData(this)
           ...
           meta.CryostatInfo = 'MPMS3, alt format';
        end  
        ...
        function [temperature, field, range] = processScanMetaDataLine(this, line)
            ...
            temperatureVal = values(1);
            fieldVal = mean([values(2), values(3)]);
            
            %idiotic bit! Switch T and H. Round T to nearest integer, it
            %fluctuates a bit and this can confuse the error checking
            %later. If you used say 3.5 K this could be an issue though!
            %It's a hack, deal with it
            field = round(temperatureVal);
            temperature = fieldVal;
            
            
            %Another stupid hack - this seems to get units of final
            %signalsize into emu later..
            %this.RescaleFactor = 1 / (field * 10000);          
