...
classdef TrimZ < squidlab.postprocess.PostProcessPipe
    
    methods
        
        function [outputScans, temperatures] = process(~, inputScans, temperatures, zRestrictionRange)
            inZRange = inputScans(:,1,1) >= zRestrictionRange(1) & ...
                        inputScans(:,1,1) <= zRestrictionRange(2); 

            outputScans = inputScans(inZRange, :, :);
        end
